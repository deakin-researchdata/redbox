...
function setFirstWizardStep() {
    ...
    var baseTitle=$("#page-heading").attr("base-title");
    if(!baseTitle)
    {
        var baseTitle=$("#page-heading").text();
        $("#page-heading").attr("base-title", baseTitle);
    }

    var keys = Object.keys(wizard_def["steps"]) ;
    $("#page-heading").text(baseTitle + " (1/" + keys.length + ")");
...
function transition_click(e)  {
	...
	if ($(e).attr('close-transition')) {
        ...
        var keys = Object.keys(wizard_def["steps"]) ;
        $("#page-heading").text($("#page-heading").attr("base-title") + " (" + (keys.indexOf(targetState)+1) + "/" + keys.length + ")");
